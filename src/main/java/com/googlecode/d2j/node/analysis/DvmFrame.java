...
public class DvmFrame<V> {
   ...
   public DvmFrame(int totalRegister) {
      this.values = (Object[])(new Object[totalRegister]);
   }
   ...
   public void execute(DexStmtNode insn, DvmInterpreter<V> interpreter) {
      if (insn.op != null) {
         ...
         case NEW_ARRAY:
         case INSTANCE_OF:
            typeStmtNode = (TypeStmtNode)insn;
         ...
         case INVOKE_POLYMORPHIC_RANGE:
            ...
            if (insn.op != Op.INVOKE_STATIC && insn.op != Op.INVOKE_STATIC_RANGE) {
               if (insn.op == Op.INVOKE_CUSTOM || insn.op == Op.INVOKE_CUSTOM_RANGE) {
                  isStatic = true;
               }
            } else {
               isStatic = true;
            ...
            String[] var15 = proto.getParameterTypes();
   ...
   public V getReg(int b) {
      return b <= this.values.length && b >= 0 ? this.values[b] : null;
   }
